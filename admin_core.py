...
import json, time, os, random, datetime
...
def get_rank_emoji(rank):
    emojis = ["❌", "🥄", "🥉", "🥈", "🥇", "🎖", "🏅", "⭐️", "🔥", "👑"]
    return emojis[min(rank, len(emojis)-1)]
...
def register_handlers(dp: Dispatcher):
    ...
    async def handle_report(msg: Message):
        ...
        if not text: return await msg.reply("❗ Укажи причину.")
        report = {
        ...
        await msg.bot.send_message(OWNER_ID, f"🚨 Репорт:\n👤 {report['from']}\n📅 {report['time']}\n📝 {report['text']}")
        
        # Если есть админ-чат — туда тоже
        if db.get("admin_chat"):
            await msg.bot.send_message(db["admin_chat"], f"🚨 Репорт:\n👤 {report['from']}\n📅 {report['time']}\n📝 {report['text']}")
